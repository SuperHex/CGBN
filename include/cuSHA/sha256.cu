...
__global__ void
kernel_sha256_init(CUDA_SHA256_CTX *ctxs, size_t N) {
...
__global__ void
kernel_sha256_update(CUDA_SHA256_CTX *ctxs,
                     const uint8_t *data,
                     size_t num_elements,
...
__global__ void
kernel_sha256_final(CUDA_SHA256_CTX *ctxs, uint8_t *out, size_t N) {
    for (int curr_thread = blockDim.x * blockIdx.x + threadIdx.x;
         curr_thread < N;
         curr_thread += blockDim.x * gridDim.x)
    {
        cuda_sha256_final(ctxs + curr_thread, out);
    }
}

namespace cuNTT {

void cuda_sha256_init(CUDA_SHA256_CTX ctxs[], size_t N) {
    kernel_sha256_init<<<N, cuNTT_TPB>>>(ctxs, N);
}

void cuda_sha256_update(CUDA_SHA256_CTX ctxs[], const cuNTT::device_mem_t *data, size_t N) {
...
void cuda_sha256_final(CUDA_SHA256_CTX ctxs[], uint8_t *out, size_t N) {
